...
local MoveToY = class("MoveToY", ActionBase)

function MoveToY:ctor(ui, fromPos, toPos, duration, onComplete)
    self.fromPos = fromPos
    self.toPos = toPos
    self.duration = duration
    self.onComplete = onComplete
    MoveToY.super.ctor(self, ui)
end

function MoveToY:tick(dt)
    ...
    local x = self._ui:get_relative_x() --self.fromPos.x + (self.toPos.x - self.fromPos.x) * progress
