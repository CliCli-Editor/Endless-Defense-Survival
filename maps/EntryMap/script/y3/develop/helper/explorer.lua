...
function M.createTimerWatcher()
    ...
    node:bindGC(y3.ctimer.loop(1, function ()
        ...
        for mod, count in pairs(counts) do
            subNodes[mod].description = string.format(GameAPI.get_text_config('#30000001#lua83'), count.alive, count.all - count.alive)
            all = all + count.all
            alive = alive + count.alive
        end
        node.description = string.format(GameAPI.get_text_config('#30000001#lua78'), alive, all - alive)
    end)):execute()
...
function M.createTriggerWatcher()
    ...
    node:bindGC(y3.ctimer.loop(1, function ()
        ...
        node.description = string.format(GameAPI.get_text_config('#30000001#lua78'), count.alive, count.all - count.alive)
    end)):execute()
