...
M.register('WUDI', {
    ...
    onCommand = function(...)
        ...
        if mainActor then
            ...
            mainActor:getUnit():add_attr(y3.const.UnitAttr['最大生命'], 100000)
            mainActor:getUnit():add_attr(y3.const.UnitAttr['生命'], 100000)
            mainActor:getUnit():add_attr(y3.const.UnitAttr['攻击速度'], 300)
            -- mainActor:getUnit():add_attr(y3.const.UnitAttr['物理攻击'], 90000)
        end
        local gold = player:get_attr("gold")
...
M.register('addTitle', {
    desc = GameAPI.get_text_config('#2045730291#lua'),
    onCommand = function(...)
        local args = { ... }
        local player = M.player
        if not player then
        ...
        local itemId = tonumber(args[1])
        local playerData = y3.userData:getPlayerData(player:get_id())
        local surviveTreasure = y3.gameApp:getLevel():getLogic("SurviveGameAchievementTitle")
        surviveTreasure:dropTitle(playerData:getId(), itemId)
    end
})
...
M.register('fastWin', {
    ...
    onCommand = function(...)
        ...
        for _, playerData in ipairs(allPlayers) do
            ...
            treasureDrop:updateStagePass(playerData:getId(), y3.userData:getCurStageId(), true, 2)
        end
        ...
        y3.ltimer.wait(1.5, function(timer, count)
            y3.player.with_local(function(local_player)
                y3.gameApp:dispatchEvent(y3.EventConst.EVENT_SURVEVE_RESULT_WIN, local_player:get_id())
                y3.game.end_player_game(local_player, y3.GameConst.VICTORY, true)
            end)
        end)
    end
})
...
M.register("addGuaiwu", {
    ...
    onCommand = function(...)
        ...
        local spawnEnemy = y3.gameApp:getLevel():getLogic("SurviveSpawnerEnemy")
        spawnEnemy:_spawnSingleEnemy(monsterCfg, points, playerData, nil, true)
    end
})
...
M.register("addDay", {
    ...
    onCommand = function(...)
        ...
        local str = tonumber(args[1])
        local SurviveGameCourse = y3.gameApp:getLevel():getLogic("SurviveGameAchievementTitle")
        SurviveGameCourse:dropTitle(player:get_id(), str)
...
M.register("addDia", {
    desc = GameAPI.get_text_config('#1199105650#lua'),
    onCommand = function(...)
        local args = { ... }
        local player = M.player
        if not player then
        ...
        local str = tonumber(args[1])
        local curDia = player:get_attr("diamond")
        player:set("diamond", curDia + str)
    end
})

M.register("addST", {
    desc = GameAPI.get_text_config('#-1519595534#lua'),
    onCommand = function(...)
        local args = { ... }
        local player = M.player
        if not player then
        ...
        local str = tonumber(args[1])
        local SurviveGameCourse = y3.gameApp:getLevel():getLogic("SurviveGameCourse")
        SurviveGameCourse:dropStageTower(player:get_id(), str)
    end
})

M.register("addGuaiwu2", {
    desc = GameAPI.get_text_config('#-590270524#lua'),
    onCommand = function(...)
        local args = { ... }
        local player = M.player
        ...
        local stageCfg = include("gameplay.config.stage_wave").get(1)
        local spawnEnemy = y3.gameApp:getLevel():getLogic("SurviveSpawnerEnemy")
        local monster = spawnEnemy:_spawnSingleEnemy(monsterCfg, points, playerData, stageCfg, true)
        if monster then
            monster:getUnit():set_attr(y3.const.UnitAttr['最大生命'], 2000000000)
            monster:getUnit():set_attr(y3.const.UnitAttr['生命'], 2000000000)
        end
    end
})

M.register("stop", {
    desc = GameAPI.get_text_config('#796475147#lua'),
    onCommand = function(...)
        local args = { ... }
        local flag = tonumber(args[1])
        local spawnEnemy = y3.gameApp:getLevel():getLogic("SurviveSpawnerEnemy")
        if flag == 1 then
            spawnEnemy:setSpawnerStop(true)
        else
            spawnEnemy:setSpawnerStop(false)
        end
    end
})
M.register("addBuff", {
    desc = GameAPI.get_text_config('#474921923#lua'),
    onCommand = function(...)
        local args = { ... }
        local buffid = tonumber(args[1])
        local unit = M.player:get_selecting_unit()
        local buff_data = {
            key = buffid,
            stacks = 1,
            time = -1
        }
        unit:add_buff(buff_data)
    end
})
...
M.register("addWExp", {
    ...
    onCommand = function(...)
        ...
        weaponSave:dropWeaponExp(player:get_id(), weaponId, exp)
    end
})
-----
